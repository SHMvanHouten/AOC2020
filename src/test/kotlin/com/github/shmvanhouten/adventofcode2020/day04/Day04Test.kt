...
class Day04Test {
    ...
    inner class Part1 {
        ...
        @Test
        internal fun `test input pt 1`() {
            assertThat(countValidPassports(testInput), equalTo(2))
        }
//
//        @Test
//        internal fun `part 1`() {
//            assertThat(countValidPassports(input), equalTo(202))
//        }
    }

    @Nested
    inner class Part2 {
        private val validPassport = Passport(
            mapOf(
                "byr" to "2002",
                "iyr" to "2011",
                "eyr" to "2021",
        ...
        @Test
        internal fun `valid passport is valid winky face`() {
            assertThat(validPassport.isValid(), equalTo(true))
        }

        @Test
        internal fun `birth year must be between 1920 and 2002`() {
            assertThat(validPassport.replace("byr", "1919").isValid(), equalTo(false))
            assertThat(validPassport.replace("byr", "2003").isValid(), equalTo(false))

            assertThat(validPassport.replace("byr", "1920").isValid(), equalTo(true))
            assertThat(validPassport.replace("byr", "2002").isValid(), equalTo(true))
        }

        @Test
        internal fun `issue year must be between 2010 and 2020`() {
            assertThat(validPassport.replace("iyr", "2009").isValid(), equalTo(false))
            assertThat(validPassport.replace("iyr", "2021").isValid(), equalTo(false))

            assertThat(validPassport.replace("iyr", "2010").isValid(), equalTo(true))
            assertThat(validPassport.replace("iyr", "2020").isValid(), equalTo(true))
        }

        @Test
        internal fun `expiration year must be between 2020 and 2030`() {
            assertThat(validPassport.replace("eyr", "2019").isValid(), equalTo(false))
            assertThat(validPassport.replace("eyr", "2031").isValid(), equalTo(false))

            assertThat(validPassport.replace("eyr", "2020").isValid(), equalTo(true))
            assertThat(validPassport.replace("eyr", "2030").isValid(), equalTo(true))
        }
        ...
        inner class Height {
            ...
            @Test
            internal fun `must be in cm or inches`() {
                assertThat(validPassport.replace("hgt", "190ba").isValid(), equalTo(false))
                assertThat(validPassport.replace("hgt", "60in").isValid(), equalTo(true))
                assertThat(validPassport.replace("hgt", "190cm").isValid(), equalTo(true))
            }

            @Nested
            inner class `Given height is in centimeters` {
                @Test
                internal fun `must be between 150 and 193`() {
                    assertThat(validPassport.replace("hgt", "150cm").isValid(), equalTo(true))
                    assertThat(validPassport.replace("hgt", "193cm").isValid(), equalTo(true))
                    assertThat(validPassport.replace("hgt", "149cm").isValid(), equalTo(false))
                    assertThat(validPassport.replace("hgt", "194cm").isValid(), equalTo(false))
                }
            }

            @Nested
            inner class `Given height is in inches` {
                @Test
                internal fun `must be between 59 and 76 inches`() {
                    assertThat(validPassport.replace("hgt", "59in").isValid(), equalTo(true))
                    assertThat(validPassport.replace("hgt", "76in").isValid(), equalTo(true))
                    assertThat(validPassport.replace("hgt", "58in").isValid(), equalTo(false))
                    assertThat(validPassport.replace("hgt", "77in").isValid(), equalTo(false))
                }
            }
        }
        ...
        inner class HairColor {
            @Test
            internal fun `starts with a #`() {
                assertThat(validPassport.replace("hcl", "@123456").isValid(), equalTo(false))
                assertThat(validPassport.replace("hcl", "#123456").isValid(), equalTo(true))
            }

            @Test
            internal fun `has six hexadecimal digits`() {
                assertThat(validPassport.replace("hcl", "#12345").isValid(), equalTo(false))
                assertThat(validPassport.replace("hcl", "#1234567").isValid(), equalTo(false))

                assertThat(validPassport.replace("hcl", "#123456").isValid(), equalTo(true))

                assertThat(validPassport.replace("hcl", "#nothex").isValid(), equalTo(false))
                assertThat(validPassport.replace("hcl", "#deadb4").isValid(), equalTo(true))
        ...
        inner class EyeColor {
            ...
            internal fun `is one of`(validColor: String) {
                assertThat(validPassport.replace("ecl", validColor).isValid(), equalTo(true))
            }

            @Test
            internal fun `is not anything else`() {
                assertThat(validPassport.replace("ecl", "bla").isValid(), equalTo(false))
                assertThat(validPassport.replace("ecl", "udg").isValid(), equalTo(false))
                assertThat(validPassport.replace("ecl", "bca").isValid(), equalTo(false))
                assertThat(validPassport.replace("ecl", "wds").isValid(), equalTo(false))
            }
        }

        @Test
        internal fun `passport id is a 9 digit number`() {
            assertThat(validPassport.replace("pid", "123456789").isValid(), equalTo(true))

            assertThat(validPassport.replace("pid", "1234567890").isValid(), equalTo(false))
            assertThat(validPassport.replace("pid", "12345678").isValid(), equalTo(false))
            assertThat(validPassport.replace("pid", "1234567ab").isValid(), equalTo(false))
        }

        @Test
        internal fun `part 1`() {
            assertThat(countValidPassports(input), equalTo(137))
        }
